...
class SceneTextManager(QObject):
    ...
    def __init__(self, 
        ...
        self.canvas.cancel_text_move_requested.connect(self.onCancelTextMove)
    ...
    def onLeftbuttonPressed(self, blk_id: int):
        ...
        self.canvas.begin_text_move_drag()
    ...
    def onTextBlkItemMoved(self):
        ...
        self._text_move_snapshot = {}
        self.canvas.end_text_move_drag()

    def onCancelTextMove(self):
        for item in self._text_move_items:
            position = self._text_move_snapshot.get(item)
            if position is None or item.scene() is not self.canvas:
                continue
