...
from qtpy.QtGui import QKeySequence, QPixmap, QImage, QHideEvent, QKeyEvent, QWheelEvent, QResizeEvent, QPainter, QPen, QPainterPath, QCursor, QNativeGestureEvent, QTextCursor
...
class Canvas(QGraphicsScene):
    ...
    cancel_text_move_requested = Signal()
    ...
    def __init__(self, parent=None):
        ...
        self.text_move_drag_active = False
    ...
    def begin_text_move_drag(self):
        self.text_move_drag_active = True

    def end_text_move_drag(self):
        self.text_move_drag_active = False
    ...
    def render_result_img(self):
        ...
        text_opacity = self.textLayer.opacity()
        text_visible = self.textLayer.isVisible()
        scale_before = self.scale_factor
        hb_pos = self.hscroll_bar.value()
        vb_pos = self.vscroll_bar.value()
        selected_before = tuple(self.selectedItems())
        focus_item_before = self.focusItem()
        focus_widget_before = QApplication.focusWidget()
        editing_item = None
        if self.textEditMode() and self.txtblkShapeControl.blk_item is not None:
            editing_item = self.txtblkShapeControl.blk_item
        editing_was_active = bool(
            editing_item is not None and editing_item.is_editting()
        )
        editing_cursor_before = (
            QTextCursor(editing_item.textCursor())
            if editing_was_active
            else None
        )
        editing_had_focus = bool(
            editing_item is not None and editing_item.hasFocus()
        )
        editing_textblkitem_before = self.editing_textblkitem
        overlay_visibility = {
            item: item.isVisible()
        ...
        try:
            ...
            if result is None or result.isNull():
                raise RuntimeError('unable to allocate result render image')
            canvas_sz = self.img_window_size()
            painter = QPainter(result)
            if not painter.isActive():
                raise RuntimeError('unable to begin result image painter')
            ...
            if scale_before != self.scale_factor:
            ...
            self.textLayer.setOpacity(text_opacity)
            self.textLayer.setVisible(text_visible)
            self.inpaintLayer.setVisible(inpaint_visible)
            if not render_succeeded:
                selected_items = tuple(
                    item for item in selected_before if item.scene() is self
                )
                if set(self.selectedItems()) != set(selected_items):
                    self.clearSelection()
                ...
                if (
                    ...
                    and editing_item.scene() is self
                ):
                    if not editing_item.is_editting():
                        editing_item.startEdit()
                    editing_item.setTextCursor(editing_cursor_before)
                self.editing_textblkitem = editing_textblkitem_before
                if focus_item_before is not None and focus_item_before.scene() is self:
                    focus_item_before.setFocus()
                elif editing_item is not None and not editing_had_focus:
                    editing_item.clearFocus()
                if focus_widget_before is not None:
                    focus_widget_before.setFocus()
    ...
    def keyPressEvent(self, event: QKeyEvent) -> None:
        ...
        elif key == Qt.Key.Key_Escape and self.text_move_drag_active:
            event.accept()
            self.cancel_text_move_requested.emit()
            grabber = self.mouseGrabberItem()
            if grabber is not None:
                grabber.ungrabMouse()
            self.end_text_move_drag()
            return
    ...
    def mouseReleaseEvent(self, event: QGraphicsSceneMouseEvent) -> None:
        ...
        if btn == Qt.MouseButton.LeftButton and self.text_move_drag_active:
            self.end_text_move_drag()
        return result
    ...
    def setTextBlockMode(self, mode: bool):
        ...
        self.text_overlay_manager.set_textblock_mode(mode)
