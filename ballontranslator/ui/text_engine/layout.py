...
def selection_segments_excluding(
    ...
    if end <= start:
        return []
    # Sweep once instead of rescanning every surviving segment per exclusion.
    segments = []
    cursor = start
    for exclusion_start, exclusion_end in sorted(exclusions):
        ...
        if cursor < exclusion_start:
            segments.append((cursor, exclusion_start))
...
class SceneTextLayout(QAbstractTextDocumentLayout):
    ...
    def fragment_format_ranges(
        ...
        """Return intersecting format runs in O(log F + K), not O(end-start)."""
        ends = self._block_fragment_ends[block_number]
