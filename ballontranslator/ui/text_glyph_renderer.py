...
def _draw_fallback(
    ...
    """Compatibility wrapper for one pathless glyph run."""
    bounds = QRectF()
    try:
        indexes = list(run.glyphIndexes())
        if indexes:
            bounds = _mapped_rect(
                run.rawFont().boundingRect(indexes[0]), transform
            )
    except (AttributeError, RuntimeError, TypeError, ValueError):
        pass
    raw_bounds = QRectF()
    try:
        indexes = list(run.glyphIndexes())
        if indexes:
            raw_bounds = run.rawFont().boundingRect(indexes[0])
    except (AttributeError, RuntimeError, TypeError, ValueError):
        pass
    _draw_fallbacks(
        painter,
        (
            FallbackGlyph(
                run, transform, bounds, raw_bounds, False
            ),
        ),
        brush,
        outline,
