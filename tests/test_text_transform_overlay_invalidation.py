...
class TextTransformOverlayInvalidationTests(unittest.TestCase):
    def tearDown(self):
        # PySide6 retains parentless offscreen views through queued Qt events.
        # Drain them between cases so later scene paints do not target wrappers
        # whose Python owners have already gone out of scope.
        for widget in QApplication.topLevelWidgets():
            widget.close()
            widget.deleteLater()
        deferred_delete = getattr(
            getattr(QEvent, 'Type', QEvent), 'DeferredDelete'
        )
    ...
    def test_manager_batch_update_coalesces_register_unregister_invalidation(self):
        scene = QGraphicsScene()
        scene.setSceneRect(QRectF(0, 0, 600, 400))
        base = QGraphicsRectItem(scene.sceneRect())
        scene.addItem(base)
    ...
    def test_canvas_export_excludes_overlays_and_restores_state(self):
        ...
        self.assertTrue(any(overlay_visibility.values()))
