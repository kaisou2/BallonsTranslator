...
class TextTransformEditingTest(unittest.TestCase):
    ...
    def test_direction_switch_preserves_transform_and_selection_direction(self):
        item, block, _scene = make_item()
        item.startEdit()
        cursor = item.textCursor()
        cursor.setPosition(7)
    ...
    def test_cursor_local_scene_round_trip_uses_item_boundary(self):
        for vertical in (False, True):
            with self.subTest(vertical=vertical):
                item, _block, _scene = make_item(
                    'Latin \u6f22\u5b57', vertical=vertical
                )
                item.setRotation(21.0)
                item.startEdit()
                cursor = item.textCursor()
                # Position 4 begins a new vertical column and its one-pixel
                # caret straddles two valid hit regions. Use an interior caret
                # for a binding-stable exact hit-test oracle.
                original_position = 3 if vertical else 4
                ...
                local_point = (
                    local_rect.topLeft()
                    + QPointF(local_rect.width() / 2.0, 0.5)
                    if vertical
                    else local_rect.center()
                )
                scene_point = item.mapToScene(local_point)
                self.assertPointAlmostEqual(
                    item.mapFromScene(scene_point), local_point
                )
    ...
    def test_ime_preedit_commit_and_cursor_query_survive_transform_update(self):
        for vertical in (False, True):
            with self.subTest(vertical=vertical):
                item, _block, _scene = make_item('IME ', vertical=vertical)
                item.startEdit()
                cursor = item.textCursor()
                ...
                self.assertEqual(item.textCursor().position(), len('IME \ud55c'))
                item.clearFocus()
                _scene.clearFocus()
