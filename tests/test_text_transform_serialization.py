...
EMPTY_PROJECT_DIR = os.path.join(FIXTURE_DIR, 'empty-project')
CANONICAL_FIELDS = (
    'horizontal_scale',
    'vertical_scale',
    'slant_angle',
...
def project_payload(blocks, *, version=2, image_info=None):
...
class TextTransformSchemaTest(unittest.TestCase):
    ...
    def test_upstream_payload_migrates_neutral_without_changing_content(self):
        ...
        migrated, _ = migrate_text_transform_payload(without_fontformat)
        self.assertEqual(
...
class ProjectPersistenceTest(unittest.TestCase):
    ...
    def seeded_project():
        ...
        project.text_transform_migration_warnings = ['keep warning state']
        return project
    ...
    def state_snapshot(project):
        return {
            ...
            for name in (
                ...
                '_idx2pagename',
                '_image_info',
                'text_transform_migration_warnings',
            )
        }
    ...
    def test_encoder_save_and_reload_emit_canonical_schema_v2(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            remigrated, warnings = migrate_text_transform_payload(second_disk_payload)
    ...
    def test_late_validation_failure_leaves_existing_state_unchanged(self):
        project = self.seeded_project()
        project.directory = EMPTY_PROJECT_DIR
        snapshot = self.state_snapshot(project)
        source = project_payload(
            {'translation': 'candidate', 'fontformat': canonical_fontformat()},
            image_info=[],
        )

        with self.assertRaisesRegex(
            ProjectNotSupportedException,
            'image_info must be an object',
        ):
            project.load_from_dict(source)

        self.assertEqual(project.directory, EMPTY_PROJECT_DIR)
        self.assertEqual(project.current_img, 'existing.png')
        for name, previous in snapshot.items():
            self.assertIs(getattr(project, name), previous)

    def test_future_json_preserves_state_and_cause(self):
        project = self.seeded_project()
        snapshot = self.state_snapshot(project)
        ...
        self.assertIn('schema version 3', str(raised.exception))
        self.assert_seed_unchanged(project, snapshot)

    def test_missing_new_project_directory_preserves_state(self):
        project = self.seeded_project()
        snapshot = self.state_snapshot(project)
        missing_directory = os.path.join(FIXTURE_DIR, 'does-not-exist')
        self.assertFalse(os.path.exists(missing_directory))
