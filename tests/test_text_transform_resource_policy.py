...
class EffectRasterItemTests(unittest.TestCase):
    ...
    def test_box_preview_never_invalidates_or_rebuilds_local_effect_cache(self):
    ...
    def test_interactive_allocation_failure_degrades_but_export_is_fatal(self):
        item = _make_item(stroke_width=0.18, shadow_radius=0.2, shadow_strength=0.8)
    ...
    def test_huge_stroke_uses_direct_vector_path_without_tile_allocation(self):
        item = _make_item(stroke_width=120.0)
    ...
    def test_export_many_visible_tiles_stream_with_two_entry_cache(self):
        item = _make_item(stroke_width=0.2)
        item.setRect(
    ...
    def test_zero_angle_scratch_null_and_cap_use_vector_bounds_fallback(self):
        expected = QRectF(2.0, 3.0, 40.0, 50.0)

        null_item = _make_item()
        with mock.patch.object(
            null_item.layout,
            'glyphInkBounds',
            return_value=expected,
        ) as vector_bounds, mock.patch.object(
            textitem_module,
            'QImage',
            side_effect=lambda *_args, **_kwargs: QImage(),
        ):
            actual = null_item._logical_ink_bounds()
        self.assertEqual(actual, expected)
        self.assertEqual(vector_bounds.call_count, 1)
        self.assertEqual(
            null_item._effect_allocation_warning_generation,
            null_item._effect_cache_generation,
        )

        capped_item = _make_item()
        capped_item.setRect(
            QRectF(0.0, 0.0, 9000.0, 9000.0), repaint=False
        )
        with mock.patch.object(
            capped_item.layout,
            'glyphInkBounds',
            ...
            textitem_module,
            'QImage',
            side_effect=AssertionError('over-cap scratch image allocated'),
        ):
            actual = capped_item._logical_ink_bounds()
        self.assertEqual(actual, expected)
        self.assertEqual(vector_bounds.call_count, 1)

    def test_vertical_stroke_rejects_unreadable_and_null_conversion_results(self):
        item = _make_item(stroke_width=0.2)
        item.setVertical(True)

        def paint_stroke():
            target = item._new_effect_pixmap(1.0)
            painter = QPainter(target)
            try:
                painter.translate(-item.boundingRect().topLeft())
                item.paint_stroke(painter)
            finally:
                painter.end()

        with mock.patch.object(
            textitem_module, 'pixmap2ndarray', return_value=None
        ):
            with self.assertRaisesRegex(
                EffectRasterAllocationError,
        ...
        with mock.patch.object(
            textitem_module, 'ndarray2pixmap', return_value=QPixmap()
        ):
            with self.assertRaisesRegex(
                EffectRasterAllocationError,
